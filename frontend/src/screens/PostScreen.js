...
import { Container, Stack, Row, Col, Form, Button, ListGroup, Badge, Dropdown, Modal } from 'react-bootstrap'
import { detailPost, commentPostAction, listuserPosts, likePostAction, unlikePostAction, savePostAction, unsavePostAction, createReportAction } from '../actions/postActions'
import parse from 'html-react-parser';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faHeart as heartunLike, faBookmark as markunsaved } from '@fortawesome/free-regular-svg-icons'
import { faAngleRight, faHeart as heartLike, faBookmark as marksaved, faEllipsisV } from '@fortawesome/free-solid-svg-icons'
...
const PostScreen = () => {
    ...
    }, [dispatch, id, successComment, successUnlike, successLike, successUnsaved, successSaved])
            ...
            <Container className='p-3'>
                <Modal show={showModal} onHide={handleClose} size="lg">
                    <Modal.Header closeButton>
                        <Modal.Title><b>Report abuse</b></Modal.Title>
                    </Modal.Header>
                    <Modal.Body>Thank you for reporting any abuse that violates our code of conduct or terms and conditions. We continue to try to make this environment a great one for everybody.
                        <Form>
                            <Form.Group className='my-3'>
                                <Form.Check type="radio" id="1" name="radioOption" label="Rude or vulgar" value="Rude or vulgar" onChange={handleRadioChange} />
                                <Form.Check type="radio" id="2" name="radioOption" label="Harassment or hate speech" value="Harassment or hate speech" onChange={handleRadioChange} />
                                <Form.Check type="radio" id="3" name="radioOption" label="Spam or copyright issue" value="Spam or copyright issue" onChange={handleRadioChange} />
                                <Form.Check type="radio" id="4" name="radioOption" label="Inappropriate listings message/category" value="Inappropriate listings message/category" onChange={handleRadioChange} />
                                <Form.Check type="radio" id="5" name="radioOption" label="Other" value="Other" onChange={handleRadioChange} />
                            </Form.Group>
                            <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
                                <Form.Label><b>Reported Post Id</b></Form.Label>
                                <Form.Control
                                    type="text"
                                    ...
                                    readOnly
                                />
                                <Form.Label><b>Posted By</b></Form.Label>
                                <Form.Control
                                    type="text"
                                    ...
                                    readOnly
                                />
                                <Form.Label><b>Message</b><br></br>
                                    Please provide any additional information or context that will help us understand and handle the situation.</Form.Label>
                                <Form.Control as="textarea" rows={3} value={reportMesaage} onChange={(e) => setReportMessage(e.target.value)} />
                            </Form.Group>
                ...
                <Row>
                        ...
                        {loading ? <Loader /> : error ? <h3>{error}</h3> :
                            ...
                            <><Container style={{ border: '1px solid lightgrey', padding: '0', borderRadius: '10px', boxShadow: '0 0 1px #333' }}>
                                ...
                                <Stack gap={3}>
                                    ...
                                    <Container className='my-3' style={{ borderBottom: '1px solid lightgrey', paddingBottom: '2rem' }}>
                                        <Row>
                                            {like && <Col><Button type='button' onClick={unlikeHandler} style={{ display: 'contents' }}><FontAwesomeIcon icon={heartLike} size='2x' color='red' /></Button><h2 style={{ display: 'inline', fontWeight: 'bold' }}>{nooflike}</h2></Col>}
                                            {!like && <Col><Button type='button' onClick={likeHandler} style={{ display: 'contents' }}><FontAwesomeIcon icon={heartunLike} size='2x' color='red' /></Button><h2 style={{ display: 'inline', fontWeight: 'bold' }}>{nooflike}</h2></Col>}

                                            {saved && <Col className='col-2'><Button type='button' onClick={unsaveHandler} style={{ display: 'contents' }}><FontAwesomeIcon icon={marksaved} size='2x' color='orange' /></Button></Col>}
                                            {!saved && <Col className='col-2'><Button type='button' onClick={saveHandler} style={{ display: 'contents' }}><FontAwesomeIcon icon={markunsaved} size='2x' color='orange' /></Button></Col>}
                                            <Col className='col-1'>
                                                <Dropdown>
                                                    ...
                                                    <Dropdown.Menu>
                                                        <Dropdown.Item onClick={handleShow}>Report Abuse</Dropdown.Item>
                                        ...
                                        <Row className='mb-3'>
                                            ...
                                            <Col>
                                                <Form onSubmit={commentHandler} style={{ display: 'flex' }}>
                                                    ...
                                                    ></Form.Control>
                                                    <Button type='submit' style={{ background: 'transparent', border: 'none' }}><FontAwesomeIcon icon={faAngleRight} size='2x' color='orange' /></Button>
                                                </Form>
                                            </Col>
                                        </Row>
                    ...
                    <Col className='col-md-4' id="side-profile-screen" style={{ marginBottom: '2rem', marginTop: '6rem' }}>
                        {loading ? <Loader /> : error ? <h3>{error}</h3> :
                            <><Container style={{ border: '1px solid lightgrey', borderRadius: '10px', boxShadow: '0 0 1px #333', padding: '1rem' }}>
                                <SideProfile post={post} />
                            </Container>

                                <Container style={{
                                    border: '1px solid lightgrey', borderRadius: '10px', boxShadow: '0 0 1px #333', padding: '1rem', marginTop: '2rem'
                                }}>
                                    <Stack gap={2}>
                                        <div><h5>More from {post && post.user ? post.user.name : null}</h5></div>
                                        <div>
                                            <ListGroup variant="flush">
                                                {loadinguserposts && <Loader />}
                                                {erroruserposts && <p>{erroruserposts}</p>}
                                                {postsofuser.filter(postid => postid._id !== id).map(userpost => (
                                                    <ListGroup.Item><LinkContainer to={`/posts/${userpost._id}`}><a href='/#'>{userpost.title}</a></LinkContainer></ListGroup.Item>
                                                ))}
                                            </ListGroup>
                                        </div>
                                    </Stack>
